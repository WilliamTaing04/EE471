# EE471 Lab 4_3 10/21/2025
# William Taing wtaing@calpoly.edu
# Description: for lab4 section 3 testing(Trajectory planning in task space using quintic polynomials)
...
def collect_data():
    ...
    robot.write_joints(joint_angles[0, 1:])
    ...
    print(f"\nTrajectory complete!")
    print(f"Planned time: {joint_angles[-1, 0]:.2f}s")
    ...
    save_to_pickle({"time": data_time, "angles": data_q, "target_poses": ee_poses, "traj_time": traj_time,"time_step": time_step, "ee_pose": data_ee_poses}, "lab4_3_data.pkl")
...
def plot_data():
    ...
    (TODO)
    """
    data = load_from_pickle("lab4_3_data.pkl")
    time_pkl = data["time"]
    angles_pkl = data["angles"]
    ee_pose_pkl = data["ee_pose"]
    
    # Joint angles
    Robot.plot_angle_list(time_pkl, angles_pkl, "Motor Angle vs Time Part3")

    # 3D trajectory
    Robot.plot_3D_trajectory_list(Robot, ee_pose_pkl,  "3D Trajectory Part3")

    # Task-space pose, velocity, and acceleration
    Robot.plot_xyz_posvelacc_list(Robot, time_pkl, ee_pose_pkl, "Task-space pose(XYZ), velocity, and acceleration Part3")
    Robot.plot_alpha_posvelacc_list(Robot, time_pkl, ee_pose_pkl, "Task-space pose(Alpha), velocity, and acceleration Part3")
